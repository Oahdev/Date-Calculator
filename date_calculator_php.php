...
if(isset($_POST["submit"])){
    $start = $_POST["start"];
    $stop = $_POST["stop"];
    if($start == "" || $stop == ""){ $error = "Enter Stop and Start date"; }
    if($start != "" && $stop != ""){
        //GETTING THE VALUE        
        $start = $_POST["start"];
        $stop = $_POST["stop"];
        ...
        for ($i=(1 + $startYear); $i <= ($stopYear - 1); $i++) { 
            ...
            elseif($i%4 != 0){ $daysInYear = 365 + $daysInYear; }
        }
        ...
        if($startYear == $stopYear){
            for ($i=(1 + $startMonth); $i < $stopMonth; $i++) { 
                if($i==1 || $i==3 || $i==5 || $i==7 || $i==8 || $i==10 || $i==12){ $daysDiffInMonthSameYear = 31 + $daysDiffInMonthSameYear; }
                if($i==4 || $i==6 || $i==9 || $i==11){ $daysDiffInMonthSameYear = 30 + $daysDiffInMonthSameYear; }
                if($i==2){
                    if($startYear%4 == 0){ $daysDiffInMonthSameYear = 29 + $daysDiffInMonthSameYear; }
                    if($startYear%4 != 0){ $daysDiffInMonthSameYear = 28 + $daysDiffInMonthSameYear; }
                }
            }
        }
        ...
        function days($days){
            if($days == 0){return $days = "Same dates"; }
            if($days == 1){return $days = $days." day"; }
            if($days > 1){return $days = $days." days"; }
        }
...
<style> input{margin-bottom: 15px;} </style>
<body>
    <div class="container">
        <div class="layout">
            ...
            <input type="text" id="days" disabled value="<?php echo $result;?>" style="width: 300px;">
            <p><?php echo $error;?></p>
        </div>
    </div>
</body>
